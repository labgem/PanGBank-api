...
def add_source_to_genomes(
    ...
    for genome_link in pangenome.genome_links:
        ...
        if source_name is None:
            logging.warning(f"No source found for genome {genome_link.genome.name}, skipping assignment.")
            continue

        genome_source = source_name_to_genome_source.get(source_name)

        if genome_source is None:
            logging.warning(f"Genome source '{source_name}' not found for genome {genome_link.genome.name}.")
            continue
    ...
    logging.info(f"Successfully assigned genome sources to genomes in pangenome {pangenome.file_name}.")
...
def create_collection_release(collection_release_info_file:Path, session:Session) -> CollectionRelease:
    ...
    if collection_release_from_db is None:
        ...
        logging.info(f'Adding a new collection release to DB: {collection.name}:{collection_release.version}')
        ...
        logging.info(f'Collection release {collection.name}:{collection_release.version} already exists in DB')
...
def parse_genomes_hash_file(genomes_md5sum_file: Path) -> dict[str, dict[str, str]]:
    ...
    if not genomes_md5sum_file.exists():
        raise FileNotFoundError(f"Genome MD5 checksum file not found: {genomes_md5sum_file}")
...
def parse_pangenome_dir(pangenome_main_dir: Path, collection_release: CollectionRelease, session: Session) -> list[Pangenome]:
    ...
    for pangenome_dir in pangenome_main_dir.iterdir():
        ...
        for genome_metric in parse_genome_metrics_file(genomes_statistics_file):
            ...
            genome_pangenome_link = session.exec(select(GenomePangenomeLink).where(and_(
            ...
            if genome_pangenome_link is None:
                ...
                pangenome_genome_link = GenomePangenomeLink.model_validate(
                    genome_metric, from_attributes=True, update={
                        "genome": genome,
                        "pangenome": pangenome,
...
def main(collection_dir: Path):
    ...
    :param collection_dir: The directory containing collection-related input files.
    :return: None
    ...
    with Session(engine) as session:
        ...
        logging.info(f"The collection release has {len(collection_release.pangenomes)} pangenomes")
        ...
        logging.info(f"The taxonomy source {taxonomy_source.name}-{taxonomy_source.version} has {len(taxonomy_source.taxa)} taxa")
        logging.info(f"The collection release {collection_release.collection.name}-{collection_release.version} has {len(collection_release.pangenomes)} pangenomes")
        
        for genome_source in genome_sources:
            logging.info(f"The genome source {genome_source.name} has {len(genome_source.genomes)} genomes")
