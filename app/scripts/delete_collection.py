...
def delete_collection(session: Session, collection_name: str) -> None:
    ...
    if collection_from_db is None:
        logging.info(f"Collection '{collection_name}' not found in the database. Deletion aborted.")
    else:
...
def delete_collection_release(session: Session, collection_name: str, release_version: str) -> None:
    """
    ...
    statement = select(CollectionRelease).join(Collection).where(
        (Collection.name == collection_name) & (CollectionRelease.version == release_version)
    )
    collection_release_from_db = session.exec(statement).first()

    if collection_release_from_db is None:
        logging.info(f"Collection release '{collection_name}' (version: {release_version}) not found in the database. Deletion aborted.")
    else:
        logging.info(f"Deleting collection release '{collection_name}' (version: {release_version}) from the database.")
